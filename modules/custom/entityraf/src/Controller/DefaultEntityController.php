...
namespace Drupal\entityraf\Controller;

use Drupal\Component\Utility\Xss;
use Drupal\Core\Controller\ControllerBase;
use Drupal\Core\DependencyInjection\ContainerInjectionInterface;
use Drupal\Core\Url;
use Drupal\entityraf\Entity\DefaultEntityInterface;
 ...
 *
 * @package Drupal\entityraf\Controller
 */
class DefaultEntityController extends ControllerBase implements ContainerInjectionInterface {
  ...
  public function revisionPageTitle($default_entity_revision) {
    $default_entity = $this->entityManager()->getStorage('default_entity')->loadRevision($default_entity_revision);
    return $this->t('Revision of %title from %date', array('%title' => $default_entity->label(), '%date' => format_date($default_entity->getRevisionCreationTime())));
  }

  /**
   * Generates an overview table of older revisions of a Default entity .
   *
   * @param \Drupal\entityraf\Entity\DefaultEntityInterface $default_entity
  ...
  public function revisionOverview(DefaultEntityInterface $default_entity) {
    ...
    foreach (array_reverse($vids) as $vid) {
      /** @var \Drupal\entityraf\DefaultEntityInterface $revision */
    ...
    $build['default_entity_revisions_table'] = array(
      '#theme' => 'table',
      '#rows' => $rows,
      '#header' => $header,
    );
